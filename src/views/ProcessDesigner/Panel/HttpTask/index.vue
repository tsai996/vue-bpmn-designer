...
<template>
  <el-collapse-item name="arg1" :title="$tu('请求')">
    <el-form-item :label="$tu('请求地址')">
      <el-input v-model="requestUrl" :placeholder="$tu('请输入请求地址')">
        <template #prepend>
          <el-select
    ...
    <el-form-item :label="$tu('请求头')">
      <Codemirror
        :rows="4"
        :max-rows="6"
        autosize
        popup
        :placeholder="$tu('请输入请求头')"
        :extensions="[jsonSupport]"
        v-model="requestHeaders"
      />
      <!--      <el-input v-model="requestHeaders" type="textarea" :rows="3" :placeholder="$tu('请输入请求头')" />-->
    </el-form-item>
    <el-form-item :label="$tu('请求体')">
      <Codemirror
        :rows="4"
        :max-rows="6"
        autosize
        popup
        :placeholder="$tu('请输入请求体')"
        :extensions="[jsonSupport]"
        v-model="requestBody"
      />
      <!--      <el-input v-model="requestBody" type="textarea" :rows="3" :placeholder="$tu('请输入请求体')" />-->
    </el-form-item>
    <el-row :gutter="10">
      <el-col :span="form?.labelPosition === 'top' ? 12 : 24">
        <el-form-item :label="$tu('忽略异常')">
          <el-switch v-model="ignoreException" active-value="true" inactive-value="false" />
        </el-form-item>
      </el-col>
      <el-col :span="form?.labelPosition === 'top' ? 12 : 24">
        <el-form-item :label="$tu('禁止重定向')">
          <el-switch v-model="disallowRedirects" active-value="true" inactive-value="false" />
        </el-form-item>
      </el-col>
      <el-col :span="form?.labelPosition === 'top' ? 12 : 24">
        <el-form-item :label="$tu('保存响应变量为JSON')">
          <el-switch
      ...
      <el-col :span="form?.labelPosition === 'top' ? 12 : 24">
        <el-form-item :label="$tu('保存响应参数为瞬时')">
          <el-switch
