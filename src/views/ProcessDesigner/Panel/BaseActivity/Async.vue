...
<template>
  <el-row :gutter="10">
    <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
      <el-form-item :label="$tu('前异步')">
        <el-switch v-model="asyncBefore" />
      </el-form-item>
    </el-col>
    <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
      <el-form-item :label="$tu('后异步')">
        <el-switch v-model="asyncAfter" />
      </el-form-item>
    </el-col>
    <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
      <el-form-item :label="$tu('排他')">
        <el-switch v-model="exclusive" :disabled="!(asyncBefore || asyncAfter)" />
      </el-form-item>
    </el-col>
  </el-row>
  <el-form-item :label="$tu('失败重试周期')" v-show="asyncBefore || asyncAfter">
    <el-input v-model="retryTimeCycle" clearable :placeholder="$tu('请输入失败重试周期')" />
  </el-form-item>
</template>
