...
<template>
  <el-collapse-item name="arg1" :title="$tu('服务')">
    <el-form-item prop="executeType" :label="$tu('执行类型')">
    ...
    <el-form-item prop="class" :label="$tu('java类')" v-if="executeType === 'class'">
      <Codemirror
        :rows="2"
        :max-rows="6"
        autofocus
        autosize
        no-wrap
        :placeholder="$tu('请输入java类')"
        v-model="javaClass"
      />
      <!--      <el-input v-model="javaClass" type="textarea" :rows="2" :placeholder="$tu('请输入java类')" />-->
    </el-form-item>
    <div v-else-if="executeType === 'expression'">
      <el-form-item prop="expression" :label="$tu('表达式')">
        <Codemirror
          :rows="2"
          :max-rows="6"
          autofocus
          autosize
          no-wrap
          :placeholder="$tu('请输入表达式')"
          :extensions="[juelLinter, closeBrackets()]"
          v-model="expression"
        />
      </el-form-item>
      <!--el-form-item prop="resultVariableName" :label="$tu('结果变量')">
        <el-input v-model="resultVariableName" :placeholder="$tu('请输入返回结果变量名')" />
      </el-form-item>-->
    </div>
    <el-form-item
    ...
    <el-form-item label-position="top">
      ...
      <el-table :data="injectFields" height="200px">
        <el-table-column prop="name" show-overflow-tooltip :label="$tu('字段名')" />
        <el-table-column prop="type" show-overflow-tooltip :label="$tu('字段类型')" />
        <el-table-column prop="value" show-overflow-tooltip :label="$tu('字段值')" />
        <el-table-column align="center" min-width="66px" :label="$t('ui.operation')">
          <template #default="{ row }">
            <el-space>
              <el-button type="primary" :icon="EditPen" link @click="editField(row)" />
              <el-popconfirm :title="$t('ui.confirmDeleteField')" @confirm="removeField(row)">
                <template #reference>
    ...
    <el-form-item prop="triggerable" :label="$tu('可触发')" v-show="false">
      <el-switch v-model="triggerable" />
    </el-form-item>
    <el-form-item prop="mapException" :label="$tu('异常映射')">
