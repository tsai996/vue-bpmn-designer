...
<template>
  <el-collapse-item name="arg1" :title="$tu('调用活动')">
    <el-form-item prop="calledElementType" :label="$tu('活动类型')">
      <el-radio-group v-model="calledElementType">
        <el-radio-button :label="$tu('流程定义Key')" value="key" />
        <el-radio-button :label="$tu('流程定义ID')" value="id" />
      </el-radio-group>
    </el-form-item>
    <el-form-item prop="calledElement" :label="$tu('调用活动')">
      <el-input
        v-model="calledElement"
        :placeholder="
          $tu(`请输入流程定义${calledElementType}`, `Enter process definition ${calledElementType}`)
        "
      />
    </el-form-item>
    <el-form-item prop="processInstanceName" :label="$tu('流程实例名称')">
      <el-input v-model="processInstanceName" :placeholder="$tu('请输入流程实例名称')" />
    </el-form-item>
    <el-form-item prop="idVariableName" :label="$tu('实例id变量名')">
      <el-input v-model="idVariableName" :placeholder="$tu('请输入实例id变量名')" />
    </el-form-item>
    <el-form-item prop="businessKey" :label="$tu('业务键')">
      <el-input v-model="businessKey" :placeholder="$tu('请输入业务键/表达式')" />
    </el-form-item>
    <el-row :gutter="10">
      <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
        <el-form-item prop="inheritBusinessKey" :label="$tu('继承业务键')">
          <el-switch v-model="inheritBusinessKey" :disabled="!!businessKey" />
        </el-form-item>
      </el-col>
      <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
        <el-form-item prop="inheritVariables" :label="$tu('继承变量')">
          <el-switch v-model="inheritVariables" />
        </el-form-item>
      </el-col>
      <el-col :span="form?.labelPosition === 'top' ? 8 : 24">
        <el-form-item prop="sameDeployment" :label="$tu('相同部署')">
    ...
    <el-form-item prop="in" label-position="top">
      ...
      <el-table :data="inData" height="200px">
        <el-table-column prop="source" show-overflow-tooltip :label="$tu('来源')" />
        <el-table-column prop="target" show-overflow-tooltip :label="$tu('目标')" />
        <el-table-column align="center" min-width="60px" :label="$t('ui.operation')">
          <template #default="{ row }">
            <el-space>
              <el-button type="primary" :icon="EditPen" link @click="editInParams(row)" />
              <el-popconfirm :title="$t('ui.confirmDeleteField')" @confirm="removeParams(row)">
                <template #reference>
    ...
    <ParamsDrawer
      ref="inParamsDrawerRef"
      :title="$tu('入参')"
      @confirm="confirmParams($event, 'In')"
    />
    <ParamsDrawer
      ref="outParamsDrawerRef"
      :title="$tu('出参')"
      @confirm="confirmParams($event, 'Out')"
    />
  </el-collapse-item>
</template>
